...
> Salesforce (the `PofClaudeVerificationController` Apex class).

## Architecture

```
LWC button ─▶ PofClaudeVerificationController (Apex)
                 │  POST callout:MarkItDown_NC/extract-fields  (PDF base64)
                 ▼
...
## Deploy on Railway (reusing the existing Named Credential)

The Salesforce side reuses `MarkItDown_NC`, which points at the Railway base URL and sends the
`x-api-key` header. Deploy this service to that same base URL (or repoint the Named Credential)
so the new path `POST /extract-fields` is reachable.
