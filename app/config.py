...
class Settings:
    ...
    # Hard time budget for a single /extract-fields request. Must stay under the Salesforce
    # synchronous callout cap (120s) so the caller gets a clean error instead of a socket timeout.
    extract_deadline_ms: int = int(os.getenv("EXTRACT_DEADLINE_MS", "100000"))

    @property
    def extract_deadline_s(self) -> float:
        return self.extract_deadline_ms / 1000.0
